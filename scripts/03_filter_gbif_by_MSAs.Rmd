...
(fungi <- read_csv('../biodiversity_in_a_concrete_jungle_data_too_big/download_fungi_2022-12-31.csv') |> mutate(taxon = 'Fungi', .before = 1))
...
tic(); (gbif_fungi_MSA <- st_par(  sf_fungi
                                 ...
                                 , n_cores = 10
                                 , sf_func = st_join
                                 , join = st_within)); toc() # ~53s w 1M points and 10 cores
# 2x checks
...
(sf_plantae <- # 8.5 million records
   read_csv('../biodiversity_in_a_concrete_jungle_data_too_big/download_plantae_2022-12-31.csv') |>
...
tic(); (gbif_plantae_MSA <- 
          st_par(  sf_plantae
                 ...
                 , n_cores = 12
                 , sf_func = st_join
                 , join = st_within)); toc() # ~140s w 8.5M points and 12 cores
...
(sf_insecta <- # ~4.1 million records
   read_csv('../biodiversity_in_a_concrete_jungle_data_too_big/download_insecta_2022-12-31.csv') |>
...
tic(); (gbif_insecta_MSA <- 
          st_par(  sf_insecta
                 ...
                 , n_cores = 12
                 , sf_func = st_join
                 , join = st_within)); toc() # ~50s w 4.1M points and 12 cores
...
(sf_mammalia <- # ~1 million records
   read_csv('../biodiversity_in_a_concrete_jungle_data_too_big/download_mammalia_2022-12-31.csv') |>
...
(sf_amphibia <- # 300k records
   read_csv('../biodiversity_in_a_concrete_jungle_data_too_big/download_amphibia_2022-12-31.csv') |>
...
gbif_amphibia_MSA %>%
  tidylog::filter(!is.na(msa_name)) %>% # ~174k remaining
  cbind(., st_coordinates(.)) %>%
  st_set_geometry(NULL) |> 
