...
ifelse( !dir.exists('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA')
       , dir.create('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA'), FALSE)
...
(fungi <- read_fst('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_downloads/download_fungi_2023-01-22.fst') |> 
...
# map! FIXME: ECUADOR?!!? POINTS IN WATER? - doesn't really matter since we are 'clipping' down here
sf_fungi |> 
  # sample_n(10000) |>
  mapview(zcol = 'year') 
...
# HALF the size as csv, ~41 Mb
gbif_fungi_MSA %>% # maybe use sfarrow? https://wcjochem.github.io/sfarrow/index.html
  tidylog::filter(!is.na(msa_name)) %>%
  cbind(., st_coordinates(.)) %>%
  st_set_geometry(NULL) |> 
  fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/gbif_fungi_MSA_', Sys.Date(), '.fst'))
...
(sf_plantae <- # 8.5 million records
   read_fst('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_downloads/download_plantae_2023-01-22.fst') |> 
...
gbif_plantae_MSA |> 
  ...
  rowid_to_column() |> 
  mutate(group = santoku::chop_evenly(rowid, intervals = 6)) |>
  ggplot(aes(percent, reorder(msa_name, percent))) +
  geom_col() +
...
# # using fst it is half! 349Mb
# gbif_plantae_MSA %>%
#   tidylog::filter(!is.na(msa_name)) %>%
#   cbind(., st_coordinates(.)) %>%
#   st_set_geometry(NULL) |> 
#   fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/gbif_plantae_MSA_'
#                         , Sys.Date(), '.fst'))
# 
...
# create directory gbif points per biodiversity group spatially filtered to MSAs
ifelse( !dir.exists('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/aves')
       , dir.create('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/aves'), FALSE)


# since aves are in seperate folder, no need for pattern matching, much safer than previous version
(aves_files <- 
...
for(i in 1:length(aves_files)){
  ...
  cbind(., st_coordinates(.)) %>%
  st_set_geometry(NULL) |> 
  fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/aves/aves_MSA_'
                        , year
                        , '_'
                        , Sys.Date(), '.fst'))
...
(sf_insecta <- # ~4.2 million records
   read_fst('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_downloads/download_insecta_2023-01-22.fst') |>
...
tic(); (gbif_insecta_MSA <- 
          st_par(  sf_insecta
                 ...
                 , n_cores = 12
                 , sf_func = st_join
                 , join = st_within)); toc() # ~2mins w 4.2M points and 12 cores

# 2x checks
gbif_insecta_MSA |> glimpse()
gbif_insecta_MSA |> tabyl(msa_name) |> tibble() |> arrange(desc(n))


# # write out using fst ~167Mb
# gbif_insecta_MSA %>%
#   tidylog::filter(!is.na(msa_name)) %>% # ~1.85M remaining
#   cbind(., st_coordinates(.)) %>%
#   st_set_geometry(NULL) |> 
#   fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/gbif_insecta_MSA_'
#                         , Sys.Date(), '.fst'))
# 
...
(sf_mammalia <- # ~1 million records
   read_fst('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_downloads/download_mammalia_2023-01-22.fst') |>
...
tic(); (gbif_mammalia_MSA <- 
          st_par(  sf_mammalia
                 ...
                 , n_cores = 12
                 , sf_func = st_join
                 , join = st_within)); toc() # ~14s w ~1M points and 12 cores

# 2x checks
gbif_mammalia_MSA |> glimpse()
gbif_mammalia_MSA |> tabyl(msa_name) |> tibble()

  
# # write out using fst ~17Mb
# gbif_mammalia_MSA %>%
#   tidylog::filter(!is.na(msa_name)) %>% # ~225k remaining
#   cbind(., st_coordinates(.)) %>%
#   st_set_geometry(NULL) |> 
#   fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/gbif_mammalia_MSA_'
#                         , Sys.Date(), '.fst'))
# 
...
(sf_reptilia <- # 400k records
   read_fst('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_downloads/download_reptilia_2023-01-22.fst') |>
...
tic(); (gbif_reptilia_MSA <- 
          st_par(  sf_reptilia
                 ...
                 , n_cores = 12
                 , sf_func = st_join
                 , join = st_within)); toc() # ~6s w ~400k points and 12 cores

# 2x checks
gbif_reptilia_MSA |> glimpse()
gbif_reptilia_MSA |> tabyl(msa_name) |> tibble() |> arrange(desc(n))


# # write out using fst ~14.7Mb
# gbif_reptilia_MSA %>%
#   tidylog::filter(!is.na(msa_name)) %>% # ~190k remaining
#   cbind(., st_coordinates(.)) %>%
#   st_set_geometry(NULL) |> 
#   fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/gbif_reptilia_MSA_'
#                         , Sys.Date(), '.fst'))
# 
...
(sf_amphibia <- # 300k records
   read_fst('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_downloads/download_amphibia_2023-01-22.fst') |>
   mutate(taxon = 'Amphibia', .before = 1) |> 
   st_as_sf(coords = c('decimallongitude', 'decimallatitude'), crs = st_crs(4326))
 )
...
gbif_amphibia_MSA |> tabyl(msa_name) |> tibble() |> arrange(desc(n))
...
#   cbind(., st_coordinates(.)) %>%
#   st_set_geometry(NULL) |> 
#   fst::write_fst(paste0('../biodiversity_in_a_concrete_jungle_data_too_big/gbif_by_MSA/gbif_amphibia_MSA_'
#                         , Sys.Date(), '.fst'))
# 
