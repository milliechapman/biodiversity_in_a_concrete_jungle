---
title: "13_cartography_study_area_fig"
...
# # San Diego locator polygon
# sd <- 
#   tibble(x = c(-117.65, -117.65, -116, -116, -117.65), y = c(32.5, 33.55, 33.55, 32.5, 32.5)) |> 
...
# make arrow
coords <- 
  tibble(lat = c(-117.65, -117.65), lon = c(33, 30)) |> 
...
i <- 
  ...
  geom_sf(data = holc_points, color = 'black', alpha = .5) +
  geom_sf(data = conus, color = 'black', fill = NA) +
  # geom_sf(data = sd_pt, color = 'red') +
  # geom_sf(data = sd, color = 'black', fill = NA, lty = 2) +
  # geom_segment(data = coords, size = 2, 
...
ii <- 
  ...
  geom_sf(data = msa, color = '#440154FF', fill = NA) +
  geom_sf(data = ua, color = '#2A788EFF', fill = NA) +
  geom_sf(data = holc) + 
  # geom_sf(data = holc, aes(fill = holc_grade)) + 
  # scale_fill_manual(values = holc_pal) +
  # geom_sf(data = holc_points, color = 'black', alpha = .5) +
  coord_sf(xlim = c(-93, -80), ylim = c(37, 41), expand = FALSE) + # clips
  theme_bw(16) +
  NULL
  
ii

# TODO add state abbrevs or locator box in i above?
```

### San Diego (or Phoenix)
```{r}

msa |> filter(msa_name == 'Phoenix-Mesa-Chandler, AZ Metro Area')
...
## C HOLC with UA vs MSA
```{r eval=FALSE, include=FALSE}

msa |> filter(msa_name == 'Indianapolis-Carmel-Anderson, IN Metro Area')
...
#   scale_fill_manual(values = holc_pal, name = 'HOLC Grade') +
#   # geom_sf(data = holc_points, color = 'black', alpha = .5) +
#   coord_sf(xlim = c(-87.05, -85), ylim = c(39, 40.4), expand = FALSE) + # clips
#   theme_bw(16) +
#   NULL
#   
# iii
...
#   # geom_sf(data = conus, color = 'black', fill = NA) +
#   geom_sf(data = state_boundaries, lwd = .75) +
#   geom_sf(data = msa, color = '#440154FF', fill = NA) + # tk
#   geom_sf(data = ua, aes(color = 'Urban Area')) +
#   scale_color_manual(values = '#2A788EFF', name = '') +
#   geom_sf(data = holc, aes(fill = holc_grade)) +
#   scale_fill_manual(values = holc_pal, name = 'HOLC Grade') +
#   # geom_sf(data = holc_points, color = 'black', alpha = .5) +
#   coord_sf(xlim = c(-87.05, -85), ylim = c(39, 40.4), expand = FALSE) + # clips
