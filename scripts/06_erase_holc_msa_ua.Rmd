...
# 
# test_ua_msa_int |> 
#   rename(ua_GEOID = GEOID, ua_name = NAME) |>            # for consistency
#   st_write('../biodiversity_in_a_concrete_jungle_data_too_big/ua_msa_int.gpkg')


# how are UA and MSA related though - imperfectly?
(test_ua_msa_int <- 
  st_read('../biodiversity_in_a_concrete_jungle_data_too_big/ua_msa_int.gpkg'))

test_ua_msa_int |> nrow()
test_ua_msa_int |> st_collection_extract("POLYGON") |> nrow()
...
# # find the most overlapping shard of a UA per MSA
# test_ua_msa_int |> 
#   filter(ua_GEOID %in% ua_ids) %>%                       # for sanity, reduce the problem
#   mutate(int_area_km2 = as.double(st_area(.) / 1e+6)) |> # get overlapping area
#   st_drop_geometry() |> 
#   as_tibble() |> 
#   group_by(ua_GEOID) |> 
#   arrange(desc(int_area_km2)) |> # put largest overlap per group on top
#   slice(1) |>                    # extract the 'most in' 
#   ungroup() |> 
