...
TODO add climate (precip and temp)?
...
# # TODO add HOLC as random SLOPE and see which is best
# preds <- 
#   comp |> 
#   # replace_na(list(completeness = 0)) |> 
...
# viz: half eye and rain cloud TODO: add stat sig?
```{r}

# https://cran.r-project.org/web/packages/ggdist/vignettes/slabinterval.html # best of three
...
aves_holc_highly_complete <-
  read_csv('working_data/highly_complete_holcs_aves_species_lists/highly_complete_holcs_aves_species_lists_2023-04-04.csv')
...
# correlations for completeness by taxon
corrs_completeness_taxon <-
  tibble(  filename = list.files('working_data/completeness', full.names = TRUE, recursive = TRUE, pattern = 'Estimators.CSV')
         , file_contents = map(filename, ~read_csv(., col_types = cols()))) |> 
  mutate(taxon = str_remove(filename, 'working_data/completeness/') |> 
           str_remove('/Estimators.CSV') |> 
  ...
  left_join(read_csv('working_data/MSA_UA_lookup.csv') |> 
              ...
              select(msa_GEOID, ua_GEOID)
            , by = 'msa_GEOID') |> 
  select(id = Area, completeness = Completeness, taxon, level) |> 
  unite('taxon', taxon, level, sep = '_') |> 
  drop_na(completeness) |> 
  pivot_wider(id, names_from = taxon, values_from = completeness, values_fn = mean) |> 
  select(-id) |> # %$% Hmisc::rcorr() # TODO ADD CONF.INT?
...
corrs_completeness_taxon |> 
  arrange(desc(value)) |> 
  filter(row_number() %% 2 != 0) # deletes even numbered rows
  
corrs_completeness_taxon |> 
  arrange(desc(value)) |> 
  filter(row_number() %% 2 != 0) |> # deletes even numbered rows
  unite('species_pair', rowname, name) |> 
  ggplot(aes(value, reorder(species_pair, value))) +
