...
tic(); mods_preds <-
  ...
  arrange(AIC) |> 
  slice(1) |> # keeps best fitting model per taxon, level, & dv. Never look at how close AICs are together
  ungroup() |> 
  mutate(preds = map(mods, ggpredict, 'holc_grade')) |> 
  unnest(preds); toc(); beep() # ~6 seconds
```
...
# viz
mods_preds |>
  mutate(y = factor(case_when( # better labels
  ...
  facet_grid(rows = vars(y), cols = vars(taxon), scales = 'free') +
  theme_bw(11) +
  theme(legend.position = 'bottom', legend.title = element_blank()) +
  labs(x = 'HOLC Grades') +
  NULL

ggsave(filename = paste0(getwd(), '/figures/mods_preds_', Sys.Date(), '.png')
       , width = 8.7, height = 10, units = 'cm'
       , dpi = 450
       , scale = 1.65
       )
