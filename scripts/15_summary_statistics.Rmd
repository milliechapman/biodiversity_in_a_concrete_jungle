...
## D Table S1: Descriptive statistics
```{r}
...
nested_data |> 
  ...
  tidylog::mutate(  richness_per_area = log(expected_richness / area_km2)
                  , strata = paste0(taxon, ': ', level)
                  ) |> 
  tidylog::select(
      `Sampling Density (log)` = log_density # dv
    , `Estimated Completeness` = completeness
    , `Expected Richness (log)` = log_richness
    , strata # strata
    , `HOLC Grade` = holc_grade   # iv
    , `Population / km^2` = pop_per_km   # covariates
    , `NDVI (mean)` = mean_ndvi
    , `Protected Open, Accessible Space (%)` = pct_pad_cover
    , `Water (%)` = pct_water
  ) |> 
  # drop_na() |> 
  gtsummary::tbl_summary(by = 'strata') |> 
  gtsummary::as_gt() ##|>
  # gt::gtsave(filename = paste0('figures/Table_S1_Summary_Statistics_'
  #                              , Sys.Date()
