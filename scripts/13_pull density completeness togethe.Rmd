...
nested_data |> # unnest(data) |> droplevels() |> group_by(taxon, level) |> nest() |> 
  drop_na() |> 
  unnest(data) |> mutate(richness_per_area = log(expected_richness / area_km2)) |> ggplot(aes(richness_per_area)) + geom_density()
```
...
fig_1c <- 
  ...
  geom_text(data = sig_labs2 |> 
              ...
              mutate(star_height = 
                       case_when(
                         level == 'family' & taxon == 'fungi' ~ 37.3
                       , level == 'family'  & taxon == 'insecta' ~ 98.6
                       , TRUE ~ star_height),
                     x_axis = case_when(
                         level == 'species' & (taxon == 'fungi' | taxon == 'insecta') ~ 1.9
                       , level == 'family'  & (taxon == 'fungi' | taxon == 'insecta') ~ 3.1
